                ...
                <div class="col-lg-8">
                    <div class="px-3">
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body p-3" id="selected_address" style="cursor: pointer;" data-bs-toggle="modal"
                                data-bs-target="#addressModal">
                                @php
                                    $primaryAddress =
                                        collect($addresses)->firstWhere('is_primary', true) ?? ($addresses[0] ?? null);
                                @endphp
                                ...
                                @else
                                    <p class="text-muted">Belum ada alamat tersedia. Tambahkan alamat terlebih dahulu.</p>
                                @endif
                            </div>
                        </div>

                        {{-- hidden input untuk address_id --}}
                        <input type="hidden" id="address_id" name="address_id" value="{{ $primaryAddress['id'] ?? '' }}">
    ...
    <script>
        ...
        function loadRates(addressId, methodId, shopId) {
            fetch("{{ route('checkout.rates') }}", {
                ...
                .then(res => res.json())
                .then(data => {
                    const rates = data[shopId] || [];
                    if (!rates.length) return;

                    // ambil rate pertama dari Biteship
                    const rate = rates[0];

                    // update ongkir di semua item toko ini

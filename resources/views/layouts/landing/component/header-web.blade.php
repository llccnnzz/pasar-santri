                    ...
                    <div class="header-action-right">
                        <div class="header-action-2">
                            <div class="search-location">
                                <select name="address_id" class="select-active" id="location-selector"
                                    onchange="setPrimary(document.getElementById('location-selector').value)">
                                    @if (auth()->user() && auth()->user()->primary_address)
                                        <option value="{{ auth()->user()->primary_address['id'] ?? '' }}">
                                            {{ auth()->user()->primary_address['subdistrict'] ?? 'Your Location' }},
                                            {{ auth()->user()->primary_address['city'] ?? '' }},
                                            {{ auth()->user()->primary_address['province'] ?? '' }}
                                        </option>
                                    @else
                            ...
                            <div class="header-action-icon-2">
                                <a href="/wishlist">
                                    ...
                                    @if (auth()->user() && $wishlistPopUp && count(json_decode($wishlistPopUp->items, true)) > 0)
                                        <span
                                            class="pro-count blue">{{ count(json_decode($wishlistPopUp->items, true)) }}</span>
                                    @endif
                                </a>
                            ...
                            <div class="header-action-icon-2">
                                <a class="mini-cart-icon" href="/cart">
                                    ...
                                    @if (auth()->user() && $cartPopUp && count(json_decode($cartPopUp->items, true)) > 0)
                                        <span
                                            class="pro-count blue">{{ count(json_decode($cartPopUp->items, true)) }}</span>
                                    @endif
                                </a>
                                        ...
                                        @if ($cartPopUp)
                                            <ul>
                                                    ...
                                                    @if ($i < 2)
                                                        <li>
                                                            <div class="shopping-cart-img">
                                                                <a href="/{{ $cartPopUpItem['slug'] }}"><img
                                                                        alt="Nest"
                                                                        src="{{ $cartPopUpItem['image'] }}" /></a>
                                                            </div>
                                                            <div class="shopping-cart-title">
                                                                <h4><a
                                                                        href="/{{ $cartPopUpItem['slug'] }}">{{ $cartPopUpItem['name'] }}</a>
                                                                </h4>
                                                                <h4><span>{{ $cartPopUpItem['quantity'] }} × </span>Rp.
                                                                    {{ number_format($cartPopUpItem['price']) }}</h4>
                                                            </div>
                                                            <div class="shopping-cart-delete">
                                                                <form
                                                                    id="delete-cart-pop-up-{{ $cartPopUpItem['id'] }}"
                                                                    style="display: none"
                                                                    action="/cart/{{ $cartPopUpItem['id'] }}"
                                                                    method="POST">
                                                                    @method('DELETE')
                                                                    @csrf
                                                                </form>
                                                                <a href="#"
                                                                    onclick="document.getElementById('delete-cart-pop-up-{{ $cartPopUpItem['id'] }}').submit()"><i
                                                ...
                                                @if (count(json_decode($cartPopUp->items, true)) > 2)
                                                    <li><a href="/cart">And
                                                            {{ count(json_decode($cartPopUp->items, true)) - 2 }}
                                                            more</a></li>
                                                @endif
                                            </ul>
                                            <div class="shopping-cart-footer">
                                                <div class="shopping-cart-total">
                                                    <h4>Total <span>Rp. {{ number_format($cartPopUpSubTotal) }}</span>
                                                    </h4>
                                                </div>
                                ...
                                <a class="mini-cart-icon" href="#">
                                    ...
                                    @if (auth()->check() &&
                                        ...
                                        <span class="pro-count blue">
                                            {{ auth()->user()->unreadNotifications()->where('type', 'App\Notifications\BuyerNotification')->count() }}
                                        </span>
                                    @endif
                                </a>
                                <a href="#"><span class="lable">Notifications</span></a>
                                    ...
                                    <div class="cart-dropdown-wrap cart-dropdown-hm2">
                                        @php
                                            $notifications = auth()
                                                ->user()
                                                ->notifications()
                                                ->where('type', 'App\Notifications\BuyerNotification')
                                        ...
                                        @if ($notifications->count() > 0)
                                            <ul>
                                                @foreach ($notifications as $notif)
                                                    ...
                                                    <li
                                                        class="{{ $notif->read_at ? 'bg-white text-muted' : 'bg-light' }}">
                                                        <div class="shopping-cart-img">
                                        ...
                                        @else
                                            <p class="text-center py-3">No Notifications</p>
                                        @endif
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="header-action-icon-2">
                                ...
                                <div class="cart-dropdown-wrap cart-dropdown-hm2 account-dropdown">
                                    <ul>
                                        ...
                                        @else
                                            <li><a href="/login"><i class="fi fi-rs-sign-in mr-10"></i>Sign In</a>
                                            </li>
                                        @endif
                                    </ul>
                                </div>
        ...
        <div class="container">
            <div class="header-wrap header-space-between position-relative">
                ...
                <div class="hotline d-none d-lg-flex">
                    <img src="/assets/imgs/theme/icons/phone-call.svg" style="height: 36px; width: 36px;"
                        alt="hotline" />
                    ...
                    <div class="header-action-2">
                        <div class="header-action-icon-2">
                            <a href="/wishlist">
                                ...
                                @if (auth()->user() && $wishlistPopUp && count(json_decode($wishlistPopUp->items, true)) > 0)
                                    <span
                                        class="pro-count white">{{ count(json_decode($wishlistPopUp->items, true)) }}</span>
                                @endif
                            </a>
                        </div>
                        <div class="header-action-icon-2">
                                ...
                                <div class="cart-dropdown-wrap cart-dropdown-hm2">
                                    @if ($cartPopUp)
                                        <ul>
                                            @foreach (json_decode($cartPopUp->items, true) as $i => $cartPopUpItem)
                                                @if ($i < 2)
                                                    <li>
                                                        ...
                                                        <div class="shopping-cart-delete">
                                                            <form id="delete-cart-pop-up-{{ $cartPopUpItem['id'] }}"
                                                                style="display: none"
                                                                action="/cart/{{ $cartPopUpItem['id'] }}"
                                                                method="POST">
                                                                @method('DELETE')
                                            ...
                                            @if (count(json_decode($cartPopUp->items, true)) > 2)
                                                <li><a href="/cart">And
                                                        {{ count(json_decode($cartPopUp->items, true)) - 2 }} more</a>
                                                </li>
                                            @endif
                                        </ul>
                                        <div class="shopping-cart-footer">
